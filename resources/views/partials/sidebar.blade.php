...
<style>
    ...
    .image-sidebar {
        width: 77%; height: 90px; object-fit: cover; border-radius: 8px; transition: opacity 0.1s;
    }
    .sidebar.collapsed .image-sidebar { opacity: 0; display: none; }

    .sidebar-toggle {
        position: absolute; right: -15px; top: 30px; transform: translateY(-50%);
        background: #fff; border: 1px solid #e0e0e0; color: var(--maroon);
        border-radius: 50%; width: 30px; height: 30px;
        display: flex; align-items: center; justify-content: center;
        cursor: pointer; transition: all var(--transition-speed);
        box-shadow: 0 2px 5px rgba(0, 0, 0, 0.1); z-index: 1001;
    }
    .sidebar.collapsed .sidebar-toggle { right: -15px; }
    .sidebar-toggle:hover { background: var(--maroon); color: white; border-color: var(--maroon); transform: scale(1.1); }
    .sidebar-toggle i { transition: transform var(--transition-speed) ease; }
    ...
    .search-input {
        width: 100%; padding: 0.6rem 2.2rem 0.6rem 1rem;
        border-radius: 8px; border: none;
        background: rgba(0, 0, 0, 0.15); color: white;
        transition: all var(--transition-speed); font-size: 0.9rem;
    }
    .search-input::placeholder { color: rgba(255, 255, 255, 0.6); }
    .search-input:focus { outline: none; background: rgba(0, 0, 0, 0.3); box-shadow: 0 0 0 2px var(--maroon-light); }
    .search-icon { position: absolute; right: 12px; top: 50%; transform: translateY(-50%); color: rgba(255, 255, 255, 0.6); }
    .sidebar.collapsed .sidebar-search { display: none; }

    /* --- Navigasi Container --- */
    .sidebar-nav-container { flex-grow: 1; overflow-y: auto; overflow-x: hidden; padding: 0 1rem; }

    /* Judul Grup */
    .sidebar-heading {
        font-size: 0.7rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;
        color: var(--sidebar-heading-color); padding: 1.5rem 0.5rem 0.5rem;
        white-space: nowrap; overflow: hidden; transition: opacity var(--transition-speed);
    }
    .sidebar.collapsed .sidebar-heading { padding-top: 1rem; padding-bottom: 0; }
    .sidebar.collapsed .sidebar-heading .sidebar-text { display: none; }

    /* --- Link Navigasi (Pill Style) --- */
    .nav-link {
        color: var(--sidebar-text-color); padding: 0.7rem 0.8rem;
        display: flex; align-items: center; border-radius: 8px; margin: 0.15rem 0;
        transition: all var(--transition-speed) ease; position: relative;
        white-space: nowrap; overflow: hidden;
    }
    .nav-link i {
        margin-right: 12px; font-size: 1.2rem; min-width: 24px; text-align: center;
        transition: all var(--transition-speed) ease;
    }
    .nav-link:hover { background: var(--sidebar-pill-hover); color: var(--sidebar-text-active); }
    .nav-link.active { background: var(--sidebar-pill-active); color: var(--sidebar-text-active); font-weight: 500; }
    .nav-link.active-parent { color: var(--sidebar-text-active); }

    /* --- Style Saat Collapsed --- */
    .sidebar.collapsed .nav-link { padding: 0.7rem 0; justify-content: center; }
    ...
    /* --- Dropdown Arrow --- */
    .sidebar-arrow { font-size: 0.8rem; margin-left: auto; transition: transform var(--transition-speed) ease; }
    .nav-link[aria-expanded="true"] .sidebar-arrow { transform: rotate(180deg); }

    /* --- Sub-Menu (Line-and-Dot Style) --- */
    .sub-menu { position: relative; padding-left: 2.1rem; margin-left: 0.8rem; }
    .sub-menu::before {
        content: ''; position: absolute; left: 0; top: 10px; bottom: 10px;
        width: 2px; background: rgba(255, 255, 255, 0.15); border-radius: 2px;
    }
    .sub-menu .nav-link { padding: 0.5rem 0.5rem; font-size: 0.9rem; position: relative; background: transparent !important; }
    .sub-menu .nav-link::before {
        content: ''; position: absolute; left: -1.3rem; top: 50%; transform: translateY(-50%);
        width: 6px; height: 6px; border-radius: 50%; background: rgba(255, 255, 255, 0.4);
        transition: all var(--transition-speed);
    }
    .sub-menu .nav-link:hover { color: var(--sidebar-text-active); }
    .sub-menu .nav-link:hover::before { background: var(--sidebar-text-active); }
    .sub-menu .nav-link.active { color: var(--sidebar-text-active); font-weight: 500; background: transparent !important; }
    .sub-menu .nav-link.active::before { background: var(--sidebar-text-active); transform: translateY(-50%) scale(1.3); }

    /* --- User Profile & Logout --- */
    .sidebar-user-profile { padding: 1rem; background: rgba(0, 0, 0, 0.1); }
    .sidebar.collapsed .sidebar-user-profile { padding: 0.5rem; }
    .sidebar.collapsed .sidebar-user-profile .sidebar-text,
    .sidebar.collapsed .sidebar-user-profile .logout-divider { display: none; }
    .sidebar.collapsed .sidebar-user-profile .logout-link { justify-content: center; }
    .sidebar.collapsed .sidebar-user-profile .logout-link .sidebar-text { display: none; }

    .logout-link { padding: 0.5rem; color: var(--sidebar-text-color); font-weight: 500; margin-bottom: 0.5rem; }
    .logout-link:hover { background: var(--sidebar-pill-hover); color: var(--sidebar-text-active); }
    .logout-divider { border-color: rgba(255, 255, 255, 0.1); margin: 0.5rem 0 1rem; }
</style>